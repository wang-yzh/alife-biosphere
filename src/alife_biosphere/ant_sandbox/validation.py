...
from .config import AntAgentConfig, AntSandboxConfig
...
class ValidationCase:
    ...
    base_nest_food: int
    base_unloads: int
    pheromone_on_nest_food: int
    pheromone_off_nest_food: int
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            "seed": self.seed,
            "base_nest_food": self.base_nest_food,
            "base_unloads": self.base_unloads,
            "pheromone_on_nest_food": self.pheromone_on_nest_food,
            "pheromone_off_nest_food": self.pheromone_off_nest_food,
...
def run_validation_cases(seeds: tuple[int, ...] = DEFAULT_VALIDATION_SEEDS) -> list[ValidationCase]:
    ...
    for seed in seeds:
        ...
        cases.append(
            ValidationCase(
                seed=seed,
                base_nest_food=base_result.world.nest.stored_food,
                base_unloads=sum(1 for event in base_result.events if event.event_type == "food_unload"),
                pheromone_on_nest_food=pheromone_on.world.nest.stored_food,
                pheromone_off_nest_food=pheromone_off.world.nest.stored_food,
...
def summarize_validation_status(cases: list[ValidationCase]) -> dict[str, object]:
    ...
    m2_status = (
        "pass"
        if all(case.base_nest_food > 0 and case.base_unloads > 0 for case in cases)
        else "fail"
    )
    m3_status = (
        "pass"
        if all(case.pheromone_on_nest_food > case.pheromone_off_nest_food for case in cases)
        else "partial"
    )
    m4_status = (
        "pass"
    ...
    return {
        ...
        "mean_base_nest_food": round(sum(case.base_nest_food for case in cases) / len(cases), 4),
        "mean_pheromone_advantage": round(
            sum(case.pheromone_on_nest_food - case.pheromone_off_nest_food for case in cases) / len(cases),
            4,
        ),
    }
