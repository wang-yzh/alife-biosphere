...
def build_showcase_config(seed: int = 7, ticks: int = 1800) -> AntSandboxConfig:
    colonies = (
        ColonyConfig(
            ...
            color="#375a7f",
            ant_count=11,
            nest=NestConfig(x=18, y=48, radius=4, initial_stored_food=12, colony_upkeep_per_ant_tick=0.0032),
        ),
        ColonyConfig(
            colony_id="shu",
            display_name="Shu",
            color="#b24a3a",
            ant_count=11,
            nest=NestConfig(x=98, y=18, radius=4, initial_stored_food=12, colony_upkeep_per_ant_tick=0.0032),
        ),
        ColonyConfig(
            colony_id="wu",
            display_name="Wu",
            color="#2f8f5b",
            ant_count=10,
            nest=NestConfig(x=80, y=66, radius=4, initial_stored_food=12, colony_upkeep_per_ant_tick=0.0032),
        ),
    )
    ants = AntAgentConfig(
        ant_count=32,
        ...
        metabolism_cost=0.047,
        hunger_return_threshold=7.2,
        starvation_grace_ticks=120,
        nest_feed_amount=4.4,
        trail_deposit=1.6,
