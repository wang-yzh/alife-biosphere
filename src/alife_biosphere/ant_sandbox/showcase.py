...
def _randomized_food_patches(seed: int) -> tuple[FoodPatchConfig, ...]:
...
def build_showcase_config(seed: int = 7, ticks: int = 1080) -> AntSandboxConfig:
    colonies = (
        ColonyConfig(
            ...
            color="#375a7f",
            ant_count=11,
            nest=NestConfig(x=16, y=50, radius=4, initial_stored_food=10, colony_upkeep_per_ant_tick=0.0035),
        ),
        ColonyConfig(
            colony_id="shu",
            display_name="Shu",
            color="#b24a3a",
            ant_count=11,
            nest=NestConfig(x=100, y=18, radius=4, initial_stored_food=10, colony_upkeep_per_ant_tick=0.0035),
        ),
        ColonyConfig(
            colony_id="wu",
            display_name="Wu",
            color="#2f8f5b",
            ant_count=10,
            nest=NestConfig(x=96, y=78, radius=4, initial_stored_food=10, colony_upkeep_per_ant_tick=0.0035),
        ),
    )
    ants = AntAgentConfig(
        ant_count=32,
    ...
    return AntSandboxConfig(
        ...
        colonies=colonies,
        food_patches=_randomized_food_patches(seed),
        ants=ants,
